      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        ...
        <div className="space-y-5">
          <div>
            ...
            <h1 className="text-2xl font-heading font-bold text-foreground leading-tight">{product.name}</h1>
            <p className="text-sm text-muted-foreground mt-1">{product.description}</p>
            <div className="text-2xl font-heading font-bold text-foreground mt-3">{formatCurrency(product.price)}</div>
          </div>
          ...
          <div className="bg-secondary/50 p-5 rounded-xl border border-border">
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div>
                ...
                <div className="flex items-center">
                  ...
                  <input type="number" value={quantity} readOnly className="w-12 h-9 bg-card border-y border-border text-center text-sm font-semibold text-foreground outline-none" />
              ...
              <div>
                ...
                <select value={duration} onChange={e => setDuration(Number(e.target.value))} className="w-full h-9 px-3 bg-card border border-border rounded-md text-sm font-medium text-foreground outline-none focus:border-primary focus:ring-1 focus:ring-primary appearance-none">
            ...
            <div className="bg-card p-4 rounded-lg border border-primary/20 space-y-2.5">
              ...
              <div className="flex justify-between items-center">
                <span className="font-heading font-bold text-foreground text-sm">Monthly Deduction</span>
                <span className="text-lg font-heading font-bold text-primary">{formatCurrency(monthlyDeduction)}</span>
              </div>
            </div>
